...
void Java_org_rocksdb_AbstractSlice_createNewSliceFromString(
  ...
  const auto* slice = new rocksdb3131::Slice(buf);
  rocksdb3131::AbstractSliceJni::setHandle(env, jobj, slice);
}
...
jint Java_org_rocksdb_AbstractSlice_size0(
  ...
  const auto* slice = reinterpret_cast<rocksdb3131::Slice*>(handle);
...
jint Java_org_rocksdb_AbstractSlice_compare0(
  ...
  const auto* otherSlice =
    reinterpret_cast<rocksdb3131::Slice*>(otherHandle);
...
void Java_org_rocksdb_AbstractSlice_disposeInternal(
    JNIEnv* env, jobject jobj, jlong handle) {
  delete reinterpret_cast<rocksdb3131::Slice*>(handle);
}
...
void Java_org_rocksdb_Slice_createNewSlice0(
  ...
  const auto* slice = new rocksdb3131::Slice((const char*)ptrData, len);
  rocksdb3131::AbstractSliceJni::setHandle(env, jobj, slice);
}
...
void Java_org_rocksdb_Slice_createNewSlice1(
  ...
  const auto* slice =
      new rocksdb3131::Slice(buf, len - 1);
...
void Java_org_rocksdb_DirectSlice_createNewDirectSlice0(
  ...
  const auto* slice = new rocksdb3131::Slice(ptrData, length);
  rocksdb3131::AbstractSliceJni::setHandle(env, jobj, slice);
}
...
void Java_org_rocksdb_DirectSlice_createNewDirectSlice1(
  ...
  const auto* ptrData =
    reinterpret_cast<char*>(env->GetDirectBufferAddress(data));
  const auto* slice = new rocksdb3131::Slice(ptrData);
  rocksdb3131::AbstractSliceJni::setHandle(env, jobj, slice);
}
...
void Java_org_rocksdb_DirectSlice_clear0(
    JNIEnv* env, jobject jobj, jlong handle) {
  auto* slice = reinterpret_cast<rocksdb3131::Slice*>(handle);
  delete [] slice->data_;
  slice->clear();
}
