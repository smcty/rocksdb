...
// calling c++ rocksdb3131::DB methods from Java side.
...
void Java_org_rocksdb_RocksDB_open__JLjava_lang_String_2(
  ...
  rocksdb3131::DB* db = nullptr;
  const char* db_path = env->GetStringUTFChars(jdb_path, 0);
  rocksdb3131::Status s = rocksdb3131::DB::Open(*opt, db_path, &db);
...
void Java_org_rocksdb_RocksDB_openROnly__JLjava_lang_String_2(
  ...
  rocksdb3131::DB* db = nullptr;
  const char* db_path = env->GetStringUTFChars(jdb_path, 0);
  rocksdb3131::Status s = rocksdb3131::DB::OpenForReadOnly(*opt,
      db_path, &db);
  env->ReleaseStringUTFChars(jdb_path, db_path);
...
jobject Java_org_rocksdb_RocksDB_open__JLjava_lang_String_2Ljava_util_List_2I(
  ...
  rocksdb3131::Status s = rocksdb3131::DB::Open(*opt, db_path, column_families,
      &handles, &db);
  env->ReleaseStringUTFChars(jdb_path, db_path);
  // free jbyte allocations
  for (std::vector<jbyte*>::size_type i = 0;
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::ListColumnFamilies
...
jobject Java_org_rocksdb_RocksDB_listColumnFamilies(
  ...
  rocksdb3131::Status s = rocksdb3131::DB::ListColumnFamilies(*opt, db_path,
      &column_family_names);
  env->ReleaseStringUTFChars(jdb_path, db_path);
  if (s.ok()) {
    // Don't reuse class pointer
    jclass jListClazz = env->FindClass("java/util/ArrayList");
    jmethodID mid = rocksdb3131::ListJni::getArrayListConstructorMethodId(env,
        jListClazz);
    ...
    for (std::vector<std::string>::size_type i = 0;
      ...
      env->CallBooleanMethod(jvalue_list,
          rocksdb3131::ListJni::getListAddMethodId(env), jcf_value);
    }
  }
  return jvalue_list;
}
...
void rocksdb_put_helper(
    JNIEnv* env, rocksdb3131::DB* db, const rocksdb3131::WriteOptions& write_options,
    rocksdb3131::ColumnFamilyHandle* cf_handle, jbyteArray jkey, jint jkey_len,
...
void Java_org_rocksdb_RocksDB_put__J_3BI_3BI(
  ...
  static const rocksdb3131::WriteOptions default_write_options =
...
void Java_org_rocksdb_RocksDB_put__J_3BI_3BIJ(
  ...
  if (cf_handle != nullptr) {
    ...
    rocksdb3131::RocksDBExceptionJni::ThrowNew(env,
        rocksdb3131::Status::InvalidArgument("Invalid ColumnFamilyHandle."));
  }
}
...
void Java_org_rocksdb_RocksDB_put__JJ_3BI_3BI(
    ...
    jbyteArray jentry_value, jint jentry_value_len) {
  auto db = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  auto write_options = reinterpret_cast<rocksdb3131::WriteOptions*>(
...
void Java_org_rocksdb_RocksDB_write0(
  ...
  rocksdb3131::DB* db = rocksdb3131::RocksDBJni::getHandle(env, jdb);
  auto* write_options = reinterpret_cast<rocksdb3131::WriteOptions*>(
      jwrite_options_handle);
  auto* wb = reinterpret_cast<rocksdb3131::WriteBatch*>(jwb_handle);

  rocksdb3131::Status s = db->Write(*write_options, wb);
...
void Java_org_rocksdb_RocksDB_write1(
  ...
  auto* write_options = reinterpret_cast<rocksdb3131::WriteOptions*>(
      jwrite_options_handle);
  auto* wbwi = reinterpret_cast<rocksdb3131::WriteBatchWithIndex*>(jwbwi_handle);
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::KeyMayExist
jboolean key_may_exist_helper(JNIEnv* env, rocksdb3131::DB* db,
    const rocksdb3131::ReadOptions& read_opt,
    rocksdb3131::ColumnFamilyHandle* cf_handle, jbyteArray jkey, jint jkey_len,
...
jboolean Java_org_rocksdb_RocksDB_keyMayExist___3BILjava_lang_StringBuffer_2(
    ...
    jobject jstring_buffer) {
  rocksdb3131::DB* db = rocksdb3131::RocksDBJni::getHandle(env, jdb);
  return key_may_exist_helper(env, db, rocksdb3131::ReadOptions(),
...
jboolean Java_org_rocksdb_RocksDB_keyMayExist___3BIJLjava_lang_StringBuffer_2(
  ...
  rocksdb3131::DB* db = rocksdb3131::RocksDBJni::getHandle(env, jdb);
  auto cf_handle = reinterpret_cast<rocksdb3131::ColumnFamilyHandle*>(
      jcf_handle);
  if (cf_handle != nullptr) {
    return key_may_exist_helper(env, db, rocksdb3131::ReadOptions(),
...
jboolean Java_org_rocksdb_RocksDB_keyMayExist__J_3BILjava_lang_StringBuffer_2(
  ...
  rocksdb3131::DB* db = rocksdb3131::RocksDBJni::getHandle(env, jdb);
  auto& read_options = *reinterpret_cast<rocksdb3131::ReadOptions*>(
...
jbyteArray rocksdb_get_helper(
    JNIEnv* env, rocksdb3131::DB* db, const rocksdb3131::ReadOptions& read_opt,
    rocksdb3131::ColumnFamilyHandle* column_family_handle, jbyteArray jkey,
    jint jkey_len) {
  jboolean isCopy;
  jbyte* key = env->GetByteArrayElements(jkey, &isCopy);
  rocksdb3131::Slice key_slice(
...
jbyteArray Java_org_rocksdb_RocksDB_get__J_3BI(
  ...
  return rocksdb_get_helper(env,
      reinterpret_cast<rocksdb3131::DB*>(jdb_handle),
      rocksdb3131::ReadOptions(), nullptr,
      jkey, jkey_len);
}
...
jbyteArray Java_org_rocksdb_RocksDB_get__J_3BIJ(
  ...
  auto db_handle = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  auto cf_handle = reinterpret_cast<rocksdb3131::ColumnFamilyHandle*>(jcf_handle);
  if (cf_handle != nullptr) {
    return rocksdb_get_helper(env, db_handle, rocksdb3131::ReadOptions(),
...
jbyteArray Java_org_rocksdb_RocksDB_get__JJ_3BI(
  ...
  return rocksdb_get_helper(env,
      reinterpret_cast<rocksdb3131::DB*>(jdb_handle),
      *reinterpret_cast<rocksdb3131::ReadOptions*>(jropt_handle), nullptr,
      jkey, jkey_len);
}
...
jbyteArray Java_org_rocksdb_RocksDB_get__JJ_3BIJ(
    ...
    jbyteArray jkey, jint jkey_len, jlong jcf_handle) {
  auto db_handle = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  auto& ro_opt = *reinterpret_cast<rocksdb3131::ReadOptions*>(jropt_handle);
  auto cf_handle = reinterpret_cast<rocksdb3131::ColumnFamilyHandle*>(jcf_handle);
  if (cf_handle != nullptr) {
...
jint rocksdb_get_helper(
    JNIEnv* env, rocksdb3131::DB* db, const rocksdb3131::ReadOptions& read_options,
    rocksdb3131::ColumnFamilyHandle* column_family_handle, jbyteArray jkey,
...
// cf multi get
jobject multi_get_helper(JNIEnv* env, jobject jdb, rocksdb3131::DB* db,
    const rocksdb3131::ReadOptions& rOpt, jobject jkey_list, jint jkeys_count,
    jobject jcfhandle_list) {
  std::vector<rocksdb3131::Slice> keys;
  std::vector<jbyte*> keys_to_free;
  std::vector<rocksdb3131::ColumnFamilyHandle*> cf_handles;

  if (jcfhandle_list != nullptr) {
    // get cf iterator
    jobject cfIteratorObj = env->CallObjectMethod(
        jcfhandle_list, rocksdb3131::ListJni::getIteratorMethod(env));

    // iterate over keys and convert java byte array to slice
    while (env->CallBooleanMethod(
        cfIteratorObj, rocksdb3131::ListJni::getHasNextMethod(env)) == JNI_TRUE) {
      jobject jobj = (jbyteArray) env->CallObjectMethod(
          cfIteratorObj, rocksdb3131::ListJni::getNextMethod(env));
      rocksdb3131::ColumnFamilyHandle* cfHandle =
          rocksdb3131::ColumnFamilyHandleJni::getHandle(env, jobj);
  ...
  // get iterator
  jobject iteratorObj = env->CallObjectMethod(
      jkey_list, rocksdb3131::ListJni::getIteratorMethod(env));

  // iterate over keys and convert java byte array to slice
  while (env->CallBooleanMethod(
      iteratorObj, rocksdb3131::ListJni::getHasNextMethod(env)) == JNI_TRUE) {
    jbyteArray jkey = (jbyteArray) env->CallObjectMethod(
       iteratorObj, rocksdb3131::ListJni::getNextMethod(env));
  ...
  std::vector<rocksdb3131::Status> s;
  ...
  // insert in java list
  for (std::vector<rocksdb3131::Status>::size_type i = 0; i != s.size(); i++) {
    if (s[i].ok()) {
      ...
      env->CallBooleanMethod(
          jvalue_list, rocksdb3131::ListJni::getListAddMethodId(env),
              jentry_value);
    } else {
      env->CallBooleanMethod(
          jvalue_list, rocksdb3131::ListJni::getListAddMethodId(env), nullptr);
    }
  }
...
jobject Java_org_rocksdb_RocksDB_multiGet__JLjava_util_List_2I(
  ...
  return multi_get_helper(env, jdb, reinterpret_cast<rocksdb3131::DB*>(jdb_handle),
      rocksdb3131::ReadOptions(), jkey_list, jkeys_count, nullptr);
}
...
jobject
  ...
  return multi_get_helper(env, jdb, reinterpret_cast<rocksdb3131::DB*>(jdb_handle),
      rocksdb3131::ReadOptions(), jkey_list, jkeys_count, jcfhandle_list);
}
...
jobject Java_org_rocksdb_RocksDB_multiGet__JJLjava_util_List_2I(
  ...
  return multi_get_helper(env, jdb, reinterpret_cast<rocksdb3131::DB*>(jdb_handle),
      *reinterpret_cast<rocksdb3131::ReadOptions*>(jropt_handle), jkey_list,
      jkeys_count, nullptr);
}
...
jint Java_org_rocksdb_RocksDB_get__J_3BI_3BIJ(
  ...
  auto cf_handle = reinterpret_cast<rocksdb3131::ColumnFamilyHandle*>(jcf_handle);
  if (cf_handle != nullptr) {
    return rocksdb_get_helper(env, db_handle, rocksdb3131::ReadOptions(), cf_handle,
        jkey, jkey_len, jentry_value, jentry_value_len);
  } else {
    rocksdb3131::RocksDBExceptionJni::ThrowNew(env,
        rocksdb3131::Status::InvalidArgument("Invalid ColumnFamilyHandle."));
...
jint Java_org_rocksdb_RocksDB_get__JJ_3BI_3BI(
  ...
  return rocksdb_get_helper(env,
      reinterpret_cast<rocksdb3131::DB*>(jdb_handle),
      *reinterpret_cast<rocksdb3131::ReadOptions*>(jropt_handle),
...
void Java_org_rocksdb_RocksDB_remove__JJ_3BI(
  ...
  auto db = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  auto write_options = reinterpret_cast<rocksdb3131::WriteOptions*>(jwrite_options);
...
void Java_org_rocksdb_RocksDB_disposeInternal(
  ...
  delete reinterpret_cast<rocksdb3131::DB*>(jhandle);
}

jlong rocksdb_iterator_helper(
    rocksdb3131::DB* db, rocksdb3131::ReadOptions read_options,
    rocksdb3131::ColumnFamilyHandle* cf_handle) {
  rocksdb3131::Iterator* iterator = nullptr;
  if (cf_handle != nullptr) {
...
jlong Java_org_rocksdb_RocksDB_iterator__J(
  ...
  auto db = reinterpret_cast<rocksdb3131::DB*>(db_handle);
  return rocksdb_iterator_helper(db, rocksdb3131::ReadOptions(),
      nullptr);
}
...
jlongArray Java_org_rocksdb_RocksDB_iterators(
  ...
  auto& read_options = *reinterpret_cast<rocksdb3131::ReadOptions*>(
        jread_options_handle);
  std::vector<rocksdb3131::ColumnFamilyHandle*> cf_handles;
  std::vector<rocksdb3131::Iterator*> iterators;
  ...
  rocksdb3131::Status s = db->NewIterators(read_options,
      cf_handles, &iterators);
  if (s.ok()) {
    jlongArray jLongArray =
        env->NewLongArray(static_cast<jsize>(iterators.size()));
    for (std::vector<rocksdb3131::Iterator*>::size_type i = 0; i < iterators.size();
         i++) {
...
jlong Java_org_rocksdb_RocksDB_createColumnFamily(
  ...
  rocksdb3131::Status s = db_handle->CreateColumnFamily(
...
void Java_org_rocksdb_RocksDB_dropColumnFamily(
  ...
  auto cf_handle = reinterpret_cast<rocksdb3131::ColumnFamilyHandle*>(jcf_handle);
  auto db_handle = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  rocksdb3131::Status s = db_handle->DropColumnFamily(cf_handle);
  if (!s.ok()) {
    rocksdb3131::RocksDBExceptionJni::ThrowNew(env, s);
  }
}
...
jlong Java_org_rocksdb_RocksDB_getSnapshot(
    JNIEnv* env, jobject jdb, jlong db_handle) {
  auto db = reinterpret_cast<rocksdb3131::DB*>(db_handle);
  const rocksdb3131::Snapshot* snapshot = db->GetSnapshot();
...
void Java_org_rocksdb_RocksDB_releaseSnapshot(
  ...
  auto db = reinterpret_cast<rocksdb3131::DB*>(db_handle);
  auto snapshot = reinterpret_cast<rocksdb3131::Snapshot*>(snapshot_handle);
...
jstring Java_org_rocksdb_RocksDB_getProperty0__JLjava_lang_String_2I(
  ...
  rocksdb3131::Slice property_slice(property, jproperty_len);
  ...
  if (!retCode) {
    rocksdb3131::RocksDBExceptionJni::ThrowNew(env, rocksdb3131::Status::NotFound());
  }
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::Flush

void rocksdb_flush_helper(
    JNIEnv* env, rocksdb3131::DB* db, const rocksdb3131::FlushOptions& flush_options,
  rocksdb3131::ColumnFamilyHandle* column_family_handle) {
  rocksdb3131::Status s;
  if (column_family_handle != nullptr) {
...
void Java_org_rocksdb_RocksDB_flush__JJ(
  ...
  auto db = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  auto flush_options = reinterpret_cast<rocksdb3131::FlushOptions*>(jflush_options);
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::CompactRange - Full

void rocksdb_compactrange_helper(JNIEnv* env, rocksdb3131::DB* db,
    rocksdb3131::ColumnFamilyHandle* cf_handle, jboolean jreduce_level,
    jint jtarget_level, jint jtarget_path_id) {

  rocksdb3131::Status s;
  rocksdb3131::CompactRangeOptions compact_options;
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::CompactRange - Range

void rocksdb_compactrange_helper(JNIEnv* env, rocksdb3131::DB* db,
    rocksdb3131::ColumnFamilyHandle* cf_handle, jbyteArray jbegin, jint jbegin_len,
  ...
  const rocksdb3131::Slice begin_slice(reinterpret_cast<char*>(begin), jbegin_len);
  const rocksdb3131::Slice end_slice(reinterpret_cast<char*>(end), jend_len);
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::GetLatestSequenceNumber
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB enable/disable file deletions
...
void Java_org_rocksdb_RocksDB_disableFileDeletions(JNIEnv* env,
    jobject jdb, jlong jdb_handle) {
  auto* db = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  rocksdb3131::Status s = db->DisableFileDeletions();
  if (!s.ok()) {
    rocksdb3131::RocksDBExceptionJni::ThrowNew(env, s);
  }
}
...
void Java_org_rocksdb_RocksDB_enableFileDeletions(JNIEnv* env,
  ...
  auto* db = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  rocksdb3131::Status s = db->EnableFileDeletions(jforce);
  if (!s.ok()) {
    rocksdb3131::RocksDBExceptionJni::ThrowNew(env, s);
  }
}
...
//////////////////////////////////////////////////////////////////////////////
// rocksdb3131::DB::GetUpdatesSince
...
jlong Java_org_rocksdb_RocksDB_getUpdatesSince(JNIEnv* env,
  ...
  auto* db = reinterpret_cast<rocksdb3131::DB*>(jdb_handle);
  rocksdb3131::SequenceNumber sequence_number =
      static_cast<rocksdb3131::SequenceNumber>(jsequence_number);
  std::unique_ptr<rocksdb3131::TransactionLogIterator> iter;
  rocksdb3131::Status s = db->GetUpdatesSince(sequence_number, &iter);
  if (s.ok()) {
