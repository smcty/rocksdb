...
vector<string> ReduceDBLevelsCommand::PrepareArgs(const string& db_path,
  ...
  ret.push_back("--" + ARG_NEW_LEVELS + "=" + rocksdb3131::ToString(new_levels));
  if(print_old_level) {
...
void DumpSstFile(std::string filename, bool output_hex, bool show_properties) {
  ...
  // no verification
  rocksdb3131::SstFileReader reader(filename, false, output_hex);
...
void DBFileDumperCommand::DoCommand() {
  ...
  std::cout << "==============================" << std::endl;
  rocksdb3131::VectorLogPtr wal_files;
